...
import pl.edu.pw.algo.Graph.Interfaces.GraphFunctions;
...
public class BasicGraphFunctions implements GraphFunctions {
    ...
    public void addEdgeToList(int index, Edge e){
        ...
        if( e == null){
            throw new IllegalArgumentException("Given Edge is null!");
        }
    ...
    public void readGraph(String path) throws IOException {
        ...
        try{
            ...
            throw new IllegalArgumentException("FILE_READ_PROBLEM: There is a problem with reading given graph, check the file with graph!");
        }
    ...
    public LinkedList<Edge> getConnectionList(int node){
        return graph.get(node) ==null ? null : (LinkedList<Edge>)graph.get(node);
    }
